...
local interactorButtonValidFunc = nil
local interactorOnFocusFunc = nil
local interactorButtonsIndex = {}
function Awake()
...
local  isOnFocus = false
function OnFocus()
    print("=======================================================")
    isOnFocus = true
    interactorOnFocusFunc()
end

function _OnFocus()
    if not isOnFocus then
       return 
    end    
    interactorButtons = {}
    interactorButtonsIndex =  {}
    local count = 1
    for i, buttonConfig in ipairs(ButtonConfigs) do
        if interactorButtonValidFunc ~= nil and interactorButtonValidFunc(i) then
            print("DouyinInteractiveObject    OnFocus    index", i)
            ...
            interactorButtonsIndex[count] = i
            count = count + 1
        else
            interactorButtons[i] = {}
        end
...
function CallMonoMethods(index)
    if interactorButtonsIndex[index] ~= nil then
        index = interactorButtonsIndex[index]
    end
...
function SetInteractorButtonValid(func)
    interactorButtonValidFunc = func
end

function SetOnFocusCallBack(func)
    interactorOnFocusFunc = func
end