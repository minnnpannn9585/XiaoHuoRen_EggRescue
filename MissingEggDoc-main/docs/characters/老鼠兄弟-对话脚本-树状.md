...
> 〔系统注〕**贵池未空**：`Mouse_PremiumSold_01`～`08` 中仍有未售；**后 6 条**（`_03`～`_08`）须 `**Mouse_PremiumPoolUnlocked`** 后入池（**1-E-B** / **1-H-B** 付 8 块 **或** **1-E-C** 对峙开池）。**Mouse_CanAffordMint8InGame**：整场剩余奶酪（钱包 + 未捡）≥ 8。
...
> 〔系统注〕`BlackCat_MintFishPending && !Mouse_MintFishPaid && !MintFish_Obtained && !Mouse_MintFishPitchShown`（**1-hub** 菜单 · **首访**；蛙败亦同入口）。播毕写 `Mouse_MintFishPitchShown`。`!Frog_PadRefused` → 菜单「给你。」/「先不给。」；`Frog_PadRefused` → 菜单「我已经找到了。」→ **1-E-F**（拒收方向费，**不**经 **1-E-B**）。再访 hub 直付 **1-E-B** 须 `!Frog_PadRefused && Mouse_CanAffordMint8InGame`。
...
└─ 描述：（鼠哥算盘停了一拍）
   ...
   鼠哥·兜售：五块那档，你买到头也摸不全。
   玩家：那怎么开？
   鼠哥·兜售：八块奶酪碎。不开价，不给看货。
   鼠弟·兜售：……这是规矩。
└─ 【菜单】
   「给你。」（Mouse_CanAffordMint8InGame）→ 1-H-B
   「算了。」→ 1-hub【回访】+【菜单】
```
...
> 〔系统注〕**1-H** 菜单「给你。」&& `Mouse_CanAffordMint8InGame`。扣 8 `CheeseCount`；写 `Mouse_PremiumPoolUnlocked`（后组池入池）。**不写** `Mouse_MintFishPaid`。
...
| **1-H-B**  | **1-H** 菜单「给你。」· `Mouse_CanAffordMint8InGame`                                                                                               | `Mouse_PremiumPoolUnlocked`（扣 8 `CheeseCount`）                        | **1-hub**                         |
| **NGPlus** | `NGPlus`                                                                                                                                     | `Mouse_PremiumPoolUnlocked`                                           | **NGPlus-hub**                    |
| **NGPlus-hub** | `NGPlus`（经轮播）                                                                                                                            | —                                                                     | 买便宜/买贵 → hub-return；告辞→结束         |


**本脚本【变量】块**：`0-A` · `1-A` · `1-B`/`1-C` 各情报条 · `1-D` · `1-E` · `1-E-B` · **1-E-C** · **1-G-P** · **1-G-T** · **1-H-B** · **NGPlus**
