Shader "SupGames/Mobile/PostProcess"
{
	...
	uniform UNITY_DECLARE_TEX2D(_LutTex);
...
#ifdef LUT
		float3 _s	= c.rgb * 0.9375h + 0.03125h;
		float2 lut	= float2(c.b / 16.0 + c.r / 256.0, _s.g);
		c			= lerp(c, UNITY_SAMPLE_TEX2D(_LutTex, lut), _LutAmount);
#if defined(BLOOM)|| defined(BLUR)
		b			= lerp(b, c, _LutAmount);
#endif
#endif
