...
Shader "Hidden/DouyinImpostor"
{
	Properties
	{
		...
		_FramesX( "Frames X", float ) = 1.0
		_FramesY( "Frames Y", float) = 1.0
	}
	...
	SubShader
	{
		...
		Pass
		{
			...
			half4 frag (VertexOutput IN, out float outDepth : SV_Depth ) : SV_Target
			{
				...
				half3 baseColor = o.Albedo;
				half3 darkColor = half3(0.9056604, 0.8330367, 0.8330367) * baseColor;
				half4 mainDir =	GetMainLightDir();
				half4 mainColor = GetMainLightColor();

				half3 shSample = SampleSH(float3(0, 0, 0));
				half splitFlag = GetEnvSplitFlag();
				half3 shColor = lerp(half3(1,1,1), (saturate(shSample) * 0.6 + 0.2), splitFlag);

				float lambert = dot(o.Normal, normalize(float3(mainDir.x, 0.6, mainDir.z)));
				half4 color = half4(lerp(baseColor, darkColor, lambert) * shColor * mainColor.rgb, o.Alpha);
